...
The GitHub Actions workflow in this fork builds the standard and 5-column Corne
targets on every push and pull request. It can also be started manually from
the repository's **Actions** tab. Download the merged `firmware` artifact after
the build finishes.

The build matrix is defined in `build.yaml`. Normal Corne firmware uses the
custom `sharp_mip` shield in its default orientation. The central, left-side
builds include ZMK Studio support.
...
Corne target. Use it when split pairing or stored settings prevent normal
operation. The same settings-reset files can be used before reflashing the
matching 5-column firmware because the hardware is the same.
